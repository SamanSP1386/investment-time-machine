...
Rate limiting is overridden to a no-op for every test by default — real
Redis-backed limiting is exercised deliberately in tests/core/test_rate_limit.py
(unit) and test_simulations.py::test_rate_limit_exceeded_returns_429 (which
overrides the dependency the other direction, to force the limit).
"""
...
from app.api.v1.dependencies import get_db_session, rate_limit_read, rate_limit_simulation
