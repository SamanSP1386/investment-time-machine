...
**M6 addition**: `ai_explanations` (migration `0003_ai_explanation_type`) gained `explanation_type` (`initial` | `follow_up`) and a nullable `question_text`, extending the existing AI Explanations domain to also cover the Financial Tutor's follow-up question-and-answer turns — not a new domain, the same reasoning as the M5 addition above. See ADR-024 (`docs/ARCHITECTURE_DECISIONS.md`).
...
- `ai_explanations`: FK to `simulations` is NOT NULL (every explanation belongs to exactly one simulation). `explanation_text` is nullable for the same reason as `simulations`' output columns — `ai_generation_status_enum` includes `pending`/`failed`, and (M6) a `failed` row's `explanation_text` stays NULL forever by design — a rejected/unsafe generation is never persisted, even partially. `explanation_type` (`ai_explanation_type_enum`: `initial`/`follow_up`, M6) and `question_text` (nullable, populated only for `follow_up`) distinguish the Explanation Engine's output from the Financial Tutor's — see ADR-024.
