...
**Risks Found**: External architecture review requested explicit verification of the auth/cookie/CSRF assumptions the frontend's `withCredentials: true` API client depends on, rather than treating ADR-018's design as self-evidently sufficient for every deployment target.

**Severity**: Medium — this review found the *local development and CI* posture fully correct, but surfaced one genuine, previously undocumented **production deployment risk** that could silently break authentication entirely (KI-039, below) — not a vulnerability, but a correctness gap serious enough to block a real production launch if unaddressed.

**Verified**:

1. **SameSite is evaluated by registrable domain ("site"), not full origin.** `localhost:3000` (frontend dev) and `localhost:8000` (backend dev) differ in port, which makes them different *origins* (relevant to CORS) but the *same site* (`localhost`, relevant to `SameSite`). This means `SameSite=Strict` cookies are correctly sent between the two in local development, despite them being on different ports — a common point of confusion, worth stating explicitly since it is not obvious from the cookie attribute alone.
2. **CORS is configured correctly for credentialed cross-origin requests.** `backend/app/main.py` sets `allow_credentials=True` with an explicit `allow_origins` list (`settings.cors_allowed_origins`, comma-separated) — never a wildcard, which the CORS spec forbids combining with credentials anyway and browsers reject outright. The default value, `http://localhost:3000`, matches this frontend's dev port exactly. `frontend/src/lib/api/client.ts` sets `withCredentials: true` to match.
3. **Cookie attributes match Founder Decision 002/ADR-018 exactly**, confirmed directly in `backend/app/api/v1/routers/auth.py::_set_session_cookies`: both the access-token cookie (`path="/"`) and the refresh-token cookie (`path=REFRESH_TOKEN_COOKIE_PATH`, scoped to `/api/v1/auth` only) are set with `httponly=True`, `samesite="strict"`, and `secure=settings.cookie_secure` (default `True`; the `False` override exists only for non-TLS local development per that setting's own code comment, and must never be set to `False` in production).
4. **No CSRF token exists, by deliberate design (ADR-018), and this frontend does nothing to work around that absence** — there is no CSRF-token-fetching code, no hidden form field, nothing. This is consistent with the backend's chosen mitigation (`SameSite=Strict` alone) and does not itself introduce a new gap; it does mean this frontend's security fully inherits whatever `SameSite=Strict` does and does not cover (see KI-039).
5. **Frontend behavior for auth failures**: `UNAUTHORIZED`/`FORBIDDEN` both map to plain-language entries in `ERROR_COPY` (`frontend/src/lib/api/errors.ts`) today. No redirect-to-login or session-refresh-retry behavior exists yet, because no Auth UI exists yet (M7 Phase 2) — this is an accurate reflection of current scope, not a gap in this phase.

**Mitigations Implemented**: None required at the frontend layer this phase — the existing design (§1-3 above) is correct as verified. KI-039 (below) is a documentation/deployment-process gap, not a code defect this phase can fix, since no deployment configuration exists yet to fix.

**Remaining Risks**: KI-039 (new — production custom-domain requirement for `SameSite=Strict` to function at all is assumed but never enforced or even stated as a deployment requirement anywhere). Everything else named in prior reviews (AI-Markdown sanitization, full CSRF/XSS review once a form/AI panel exists) remains deferred, unchanged from the M7 Phase 1 entry above.

**Threats Deferred**: Unchanged from the M7 Phase 1 entry — a full CSRF/XSS review still waits on a real form or AI-rendered content existing to review.
