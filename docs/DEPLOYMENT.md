...
2. Watch the deploy log. You should see, in order:
   ...
   ```
   The catalog ingestion step makes real, live HTTP requests to Yahoo's public chart endpoint from Render's network and can take a while (ten symbols' full price history). It is capped at `CATALOG_INGESTION_TIMEOUT_SECONDS` (default 90s, `backend/scripts/start.sh`) — if it logs a `WARNING: starter catalog ingestion timed out after 90s` or `WARNING: starter catalog ingestion failed` line instead of "complete" — most likely a Yahoo rate-limit blip or slow/unreachable response on Render's network (`docs/KNOWN_ISSUES.md` KI-044) — the API still starts within that bounded time regardless (this step is deliberately non-fatal and now wall-clock-bounded, after a real incident where an unbounded hang here kept uvicorn from ever opening its port until Render's own deploy timeout killed the whole deploy); the `assets` table stays empty, so the **next** deploy or restart retries automatically. Manually redeploying is the fastest way to retry sooner.
