...
## [0.20.0] — 2026-07-23 — M7 Phase 4: Educational AI Panel (Founder Decision 015, Groq Provider Swap)

### Added
- **"Ask about this result" panel** (`frontend/src/components/simulation-result/results-sections.tsx::AskAboutThisResult`) — a collapsed disclosure section on the Results page, positioned directly after The Proof, wired to the existing Financial Tutor follow-up endpoint (`POST /api/v1/simulations/{id}/explanations/questions`, Founder Decision 003/004). A text input, a submit button (existing `Button`/`Input` craft and loading states), and a suggested-questions row ("Why did dividends matter here?", "What does CAGR mean?", "What does inflation adjustment do to this number?") that fills the input on click without submitting. Focus moves to the input when the panel opens; the answer region is `aria-live="polite"`. No conversation history is fetched or persisted across page loads — each question is an independent request (`useAskQuestion`, a plain `useMutation`, never cached under a query key).
- **`ExplanationResponse` type** (`frontend/src/types/api.ts`), **`askFollowUpQuestion` endpoint** (`frontend/src/lib/api/endpoints/explanations.ts`), **`useAskQuestion` hook** (`frontend/src/hooks/use-ask-question.ts`).
- **Founder Decision 015 (Option D), implemented**: `app.api.v1.dependencies.rate_limit_ai` now distinguishes anonymous from authenticated callers — anonymous: 5/min + a new 15/day cap, keyed by IP; authenticated: the Founder Specification's unchanged 20/min plus a new 150/day cap, keyed by user id (a real, stable identifier now exists, an improvement over IP-keying for this group). `RateLimitExceededError` gained a `window: "minute" | "day"` field; the exception handler now returns "come back tomorrow" copy for a daily-cap breach and keeps the existing "try again later" copy for a per-minute one — the friendly, non-punitive, window-specific messaging FD-015 clause 5 requires. New settings: `rate_limit_ai_anonymous_per_minute`, `rate_limit_ai_anonymous_per_day`, `rate_limit_ai_authenticated_per_day`.
- **`GroqProvider`** (`backend/app/ai/providers/groq_provider.py`) — implements the existing `AIProvider` Protocol against Groq's OpenAI-compatible `/chat/completions` REST endpoint via `httpx` (already a project dependency; no new SDK added).

### Changed
- **AI provider: Anthropic → Groq (Founder Decision 019, supersedes Founder Decision 003's vendor selection only)**. This project's only deployment target (Render free tier) has no budget for Anthropic's paid-only API; Groq's free tier is real and sufficient for this feature's bounded, short-answer use case. `AnthropicProvider` removed; `Settings.ai_provider` now accepts `"none" | "groq"` (was `"none" | "anthropic"`); `Settings.groq_api_key` (env `GROQ_API_KEY`) replaces the generic `ai_provider_api_key`/`AI_PROVIDER_API_KEY`; default model `llama-3.1-8b-instant` (was `claude-3-5-haiku-20241022`). The `anthropic` SDK dependency is dropped from `backend/requirements.txt`. Every other part of Founder Decision 003 — the privacy allowlist, the fixed system prompts and prompt-injection defenses, the post-generation safety gates (numeric-integrity + advice-language checks), caching/regeneration caps, and the fixed AI-unavailable fallback message — is unchanged; `app/ai/prompt.py`, `app/ai/safety.py`, and `app/ai/service.py` are byte-for-byte unmodified by this swap, confirming FD-003's provider abstraction is genuinely swappable.
- `render.yaml`, `.env.example` — `AI_PROVIDER`/`GROQ_API_KEY` documentation updated to match.

### Fixed
- **A real, previously-latent bug in `app.core.rate_limit.InMemoryRateLimiter`**, found while writing this milestone's own daily-cap test suite: `_prune` compared raw window-*index* numbers across entries that could now be computed under two different `window_seconds` values (60s and, new this milestone, 86400s) sharing one class-level `_store` — a 60s-windowed prune call was silently deleting the 86400s-windowed daily-cap entry on almost every request, resetting the daily counter back to 1 nearly every time and making the daily cap never actually trigger. Fixed by pruning against each entry's own stored wall-clock expiry instead of comparing incompatible window-index numbers; every pre-existing single-window (60s-only) call site (simulation/read/auth buckets) is unaffected, confirmed by the full existing rate-limit suite passing unmodified plus a new regression test.

### Tests
- Backend: `tests/ai/test_providers.py` rewritten for `GroqProvider` (mocked `httpx.Client`, no live network call). `tests/core/test_ai_config.py` rewritten for the `groq_api_key`/`ai_provider="groq"` startup guard. `tests/api/test_rate_limit_ai.py` (new, 9 tests) — direct dependency-level coverage of the anonymous/authenticated split, both windows, independent keying (per-user, per-IP, and confirmed never shared between the two tiers). `tests/api/test_explanations.py` extended with 2 HTTP-level tests confirming the minute-vs-day friendly-message distinction. `tests/core/test_rate_limit.py` extended with the `InMemoryRateLimiter` cross-window-size regression test. Full suite: 237 passed, 126 skipped (pre-existing Redis/Docker-integration skips — Postgres was reachable this session and fully exercised by the DB-integration suite; Redis and the Docker daemon were not), `ruff check .`/`black --check .` clean across the full repo.
- Frontend: `use-ask-question.test.tsx` (3 tests) and `ask-about-this-result.test.tsx` (8 tests, new) — suggested-question fill, disabled-until-non-empty submit, successful-answer rendering, the `generation_status: "failed"` calm/non-error rendering (Founder Decision 003's explicit rule), rate-limit and generic-error copy, and open-panel focus management. `simulation-result-client.test.tsx` updated to mock the new `useAskQuestion` hook (unaffected page-level assertions otherwise). Full suite: 61/61 files, 373 passed/4 skipped (pre-existing, unrelated), `eslint .`/`tsc --noEmit` clean, `next build` verified clean with `.env.local` moved aside and only `NEXT_PUBLIC_API_BASE_URL` set (CI-parity, `GROQ_API_KEY`/`AI_PROVIDER` absent — confirms graceful degradation does not break the build).
- **Disclosed limitation**: no live call was made against the real Groq API in this session (no `GROQ_API_KEY` available in this environment) — the provider swap is verified via a fully mocked test suite plus the graceful-degradation (`AI_PROVIDER=none`) path, which is exercised for real. Live verification against a real key is deferred to whoever holds one, post-deploy.
