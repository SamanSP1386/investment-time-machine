...
- **Severity**: Medium
- **Status**: Open — full resolution approved (Founder Decision 014, Option A), implementation scheduled but not yet built
...
- **M7 Phase 3B update (2026-07-19)**: Founder Decision 014 approves the full fix (Option A — persist `growth_series` at creation, backfill existing completed simulations, version against `calculation_version`), reopening this issue's status from "partially resolved" to "Open" so it is not mistaken for closed. Not implemented in M7 Phase 3B itself, which built only the non-chart Results foundation and had no need to read `growth_series` yet. Planned Resolution below updated accordingly. `calculation_version` was separately exposed on `SimulationResponse` in this same pass (ADR-036) — an unconditionally safe, independent schema addition, not part of the persistence fix itself.
- **Planned Resolution**: Implement Founder Decision 014 (Option A) alongside the milestone that builds the growth chart (M7 Phase 3C or equivalent) — new `simulations.growth_series` column, backfill script, repository/engine wiring to read-through the persisted column on `GET` instead of recomputing.
- **Resolution Date**: — (reopened 2026-07-19; the 2026-07-10 partial resolution above remains accurate for what M4 itself shipped)
...
- **Status**: Open (mitigated for local development — see Resolution)
- **Resolution (mitigation, not a fix to the underlying cause)**: Added `DevSeedProvider` (`backend/app/ingestion/providers/dev_seed_provider.py`, `--provider dev_seed`) — a small, deterministic, clearly-synthetic local fixture provider for AAPL/SPY/BTC-USD, going through the exact same normalization/validation/repository/audit pipeline every real provider does, so manual frontend/Simulator testing is unblocked without depending on yfinance's current rate-limit state. See ADR-035 for the full design and its production/mislabeling safeguards. This does not fix yfinance's actual rate-limiting behavior — it only provides a reliable local-dev alternative.
- **Planned Resolution**: (1) Bump `yfinance` past 0.2.44 once a version with more resilient crumb/cookie negotiation (backoff, alternate strategies, or `curl_cffi` browser-TLS impersonation, adopted in later yfinance releases) can be verified against a live, non-rate-limited window — this is the actual fix, not `dev_seed`. (2) Consider a bounded retry-with-backoff specifically for `ProviderUnavailableError`-class failures at the orchestrator level (relates to KI-015's existing "no retry/backoff" gap — this is a second, concrete motivating case for that same planned work, not a new independent request). (3) Re-run the M3 split-consistency verification runbook (KI-016) once yfinance ingestion is reliably working again — that runbook depends on exactly the provider path this issue currently blocks.
- **Resolution Date**: —
...
- **Status**: Open — root-caused, not yet fixed. A founder decision on which layer to correct (and whether a `calculation_version` bump + backfill of existing rows is required) is drafted in `docs/FOUNDER_DECISIONS.md` and awaiting founder sign-off before any fix is implemented.
- **Resolution Date**: —
