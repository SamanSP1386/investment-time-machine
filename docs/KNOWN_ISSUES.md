...
- **Severity**: Medium-High (not yet a live bug — no deployment exists yet — but a real, production-launch-blocking gap if not resolved before go-live; the failure mode, if it occurred, would be silent and total, not a partial degradation).
- **Status**: Open
- **Planned Resolution**: Before any production deployment, either (a) provision a custom domain with the frontend and backend as sibling subdomains of the same registrable parent (e.g. `app.example.com` / `api.example.com`), confirmed same-site under `SameSite=Strict`, or (b) if a shared parent domain is genuinely not available, revisit ADR-018's cookie strategy for cross-site deployment (e.g. `SameSite=None; Secure` plus a real CSRF token, reopening the exact tradeoff ADR-018 originally avoided). Must be resolved and verified against the actual chosen production hosting setup, not assumed, before `.claude/SECURITY_POLICY.md`'s production-readiness bar is considered met.
- **Resolution Date**: —
