...
### 4b. Column bound: `NUMERIC(14, 6)`, widened from `NUMERIC(10, 6)` (KI-050)

`total_return_percentage`/`cagr_percentage` were originally `NUMERIC(10, 6)` (max magnitude `9999.999999`, roughly a 100x return) — a real, live-reproduced ceiling: `$1,000 in AAPL, 2000-01-03 → today` computes a correct `total_return_percentage` of `31449.606043` and overflowed it at `session.flush()`, discarding an already-correct calculation with an opaque `500`. Widened to `NUMERIC(14, 6)` (`backend/alembic/versions/0006_widen_percentage_columns.py`) — max magnitude `99999999.999999`, roughly a 1,000,000x (~100,000,000%) return, six orders of magnitude above the worst plausible real return in this platform's own asset catalog: Bitcoin (the single most volatile asset ingested, `app.ingestion.seed_real_catalog`), from its earliest Yahoo-ingestable daily close (~$457 on 2014-09-17) to an extremely bullish future price of $1,000,000/BTC, is only a ~218,800% return — comfortably inside the new bound. Scale (6 decimal places, `PERCENTAGE_QUANTUM`) is unchanged; only the integer-digit headroom grew. No backfill was needed — a value that overflowed the old bound was never stored (the write that would have produced it always rolled back), so there was nothing to rescale, only a ceiling to raise.
